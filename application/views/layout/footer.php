<?php if ($this->session->userdata('logged_in') == true) : ?>
        ...
        <div class="container-fluid">
            <div class="d-flex align-items-center justify-content-between small">
                <!-- <div class="text-muted">Copyright &copy; <?= date("Y"); ?><a href="<?= base_url(); ?>" class="ml-1"><?= $appname = (empty($dataapp['nama_app_absensi'])) ? 'Absensi Online' : $dataapp['nama_app_absensi']; ?></a>
                    <div class="d-inline">Powered By<a href="https://www.instagram.com/nafi_muzakky/" class="ml-1">Nafi Muzakky</a></div>
                </div> -->
    ...
    <?php if ($this->session->userdata('role_id') == 1) : ?>
        ...
        <script>
            var table_absensi = $('#list-absensi-all').DataTable({
                "ajax": {
                    ...
                    "bDestroy": true,
					data: function(d) {
						// Tambahkan data yang ingin dikirim bersama request POST
						d.tgl_mulai = $('#form-cari-absensi input[name="tgl_mulai"]').val();
						d.tgl_selesai = $('#form-cari-absensi input[name="tgl_selesai"]').val();
						// Kamu bisa menambahkan data lain sesuai kebutuhan
					}
                },
                rowCallback: function(row, data, iDisplayIndex) {
                    $('td:eq(0)', row).html();
			...
			$('#form-cari-absensi').submit(function (e) { 
				e.preventDefault();
