...
class Ajax extends CI_Controller
{
    ...
    { //data absen by JSON object
		...
		$tgl_mulai = empty($input['tgl_mulai']) ? '' : $input['tgl_mulai'];
		$tgl_selesai = empty($input['tgl_selesai']) ? '' : $input['tgl_selesai'];
        ...
        if ($dataabsen == 'datapgw') {
            ...
            foreach ($query->result() as $r) {
                $data[] = [
                    ...
                    // $r->npwp,
                    $r->jenis_kelamin,
                    ($r->role_id == 1) ? '<span class="badge badge-danger ml-1">Administrator</span>' : (($r->role_id == 2) ? '<span class="badge badge-primary ml-1">Pegawai</span>' : (($r->role_id == 3) ? '<span class="badge badge-success ml-1">Magang</span>' : '<span class="badge badge-secondary ml-1">Tidak Ada Role</span>')),
                    ...
                    (($query->num_rows() > 1 && $r->role_id != 1) || $check_admin->num_rows() > 1) ?
                        ...
                        <button id="detailpegawai" class="btn btn-primary view-pegawai" data-pegawai-id="' . $r->id_pegawai . '" title="Lihat User"><span class="fas fa-fw fa-address-card"></span></button>
                        <button class="btn btn-danger delete-pegawai" title="Hapus User" data-pegawai-id="' . $r->id_pegawai . '"><span class="fas fa-trash"></span></button>
                        <button class="btn btn-warning edit-pegawai" title="Edit User" data-pegawai-id="' . $r->id_pegawai . '"><span class="fas fa-user-edit"></span></button>
                        <button class="btn btn-secondary activate-pegawai" title="Verifikasi User" data-pegawai-id="' . $r->id_pegawai . '"><span class="fas fa-user-check"></span></button>
					...
					STR_TO_DATE(
						REPLACE(
							REPLACE(
													...
													REPLACE(
														REPLACE(
															REPLACE(
																REPLACE(
																	REPLACE(
																		TRIM(SUBSTRING_INDEX(tgl_absen, ',', -1)),
																		'Januari', 'January'),
																	'Februari', 'February'),
																'Maret', 'March'),
															'April', 'April'),
								...
								'November', 'November'),
							'Desember', 'December'),
						'%d %M %Y') tgl_absen_convert
					FROM db_absensi 
					) a";
				$conditions = array();
				$parameters = array();
				if (!empty($tgl_mulai) && !empty($tgl_selesai)) {
					$conditions[] = " tgl_absen_convert BETWEEN '$tgl_mulai' AND '$tgl_selesai'";
				}
				if ($conditions) {
